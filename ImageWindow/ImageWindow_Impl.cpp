...
BEGIN_MESSAGE_MAP(CImageWindow_Impl, CDialog)
	//{{AFX_MSG_MAP(CImageWindow_Impl)
	ON_WM_CONTEXTMENU()
	...
	ON_WM_PAINT()
	//}}AFX_MSG_MAP
	ON_COMMAND(ID_DISPLAYCURRENTPOS, &CImageWindow_Impl::OnDisplaycurrentpos)
	ON_COMMAND(ID_CROSSMARK, &CImageWindow_Impl::OnCrossmark)
	ON_UPDATE_COMMAND_UI(ID_CROSSMARK, &CImageWindow_Impl::OnUpdateCrossmark)
	ON_COMMAND(ID_POPUP_ZOOM2X, &CImageWindow_Impl::OnZoom2x)
	ON_COMMAND(ID_POPUP_ZOOM5X, &CImageWindow_Impl::OnZoom5x)
	ON_COMMAND(ID_POPUP_ZOOM10X, &CImageWindow_Impl::OnZoom10x)
	ON_WM_RBUTTONDOWN()
	ON_WM_RBUTTONUP()
	ON_COMMAND(ID_REMOVEALLOVERLAY, &CImageWindow_Impl::OnRemovealloverlay)
	ON_UPDATE_COMMAND_UI(ID_REMOVEALLOVERLAY, &CImageWindow_Impl::OnUpdateRemovealloverlay)
	ON_COMMAND(ID_POPUP_SHOWTOOLBAR, &CImageWindow_Impl::OnShowtoolbar)
	ON_NOTIFY_EX_RANGE(TTN_NEEDTEXTW, 0, 0xFFFF, &CImageWindow_Impl::OnToolTipText)
	ON_NOTIFY_EX_RANGE(TTN_NEEDTEXTA, 0, 0xFFFF, &CImageWindow_Impl::OnToolTipText)
	ON_COMMAND(ID_POPUP_SHOWMINIMAP, &CImageWindow_Impl::OnPopupShowminimap)
	ON_COMMAND(ID_POPUP_SHOWPIXELMETER, &CImageWindow_Impl::OnPopupShowpixelmeter)
	ON_COMMAND(ID_EXPORT_FILE, &CImageWindow_Impl::OnExportFile)
	ON_COMMAND(ID_EXPORT_CLIPBOARD, &CImageWindow_Impl::OnExportClipboard)
	ON_COMMAND(ID_POPUP_SCREENSHOT, &CImageWindow_Impl::OnPopupScreenshot)
	ON_UPDATE_COMMAND_UI(ID_POPUP_SCREENSHOT, &CImageWindow_Impl::OnUpdatePopupScreenshot)
	ON_UPDATE_COMMAND_UI(ID_EXPORT_FILE,	&CImageWindow_Impl::OnUpdateExportFile)
	ON_UPDATE_COMMAND_UI(ID_EXPORT_CLIPBOARD, &CImageWindow_Impl::OnUpdateExportClipboard)
END_MESSAGE_MAP()
...
CImageWindow_Impl::CImageWindow_Impl(CImageWindowEvt *pHandler, CWnd *pParent)
	: CDialog(CImageWindow_Impl::IDD, pParent)
...
/////////////////////////////////////////////////////////////////////////////
// CImageWindow_Impl::~CImageWindow_Impl - Destructor

CImageWindow_Impl::~CImageWindow_Impl()
{
...
void CImageWindow_Impl::SetEventHandler(CImageWindowEvt *pHandler)
{
	m_pEventHandler = pHandler;
}

BOOL CImageWindow_Impl::FireEventLButtonDblClk(UINT nFlags, long ptx, long pty)
{
...
BOOL CImageWindow_Impl::FireEventLButtonUp(UINT nFlags, long ptx, long pty)
{
	if(m_pEventHandler == NULL)	return FALSE;
...
BOOL CImageWindow_Impl::FireEventLButtonDown(UINT nFlags, long ptx, long pty, BOOL bMinimapHit)
{
	if(m_pEventHandler == NULL)	return FALSE;
...
BOOL CImageWindow_Impl::FireEventMouseMove(UINT nFlags, long ptx, long pty)
{
	if(m_pEventHandler == NULL)	return FALSE;
...
BOOL CImageWindow_Impl::FireEventRButtonUp(UINT nFlags, long ptx, long pty, BOOL bOnContextMenuCall)
{
	if(m_pEventHandler == NULL)	return FALSE;
...
BOOL CImageWindow_Impl::FireEventRButtonDown(UINT nFlags, long ptx, long pty)
{
	if(m_pEventHandler == NULL)	return FALSE;
...
void CImageWindow_Impl::FireEventROIChanged(long nROIID)
{
...
void CImageWindow_Impl::FireEventExtPopupMenu(long nCmdID, double fx, double fy)
{
	if(m_pEventHandler == NULL)	return;
...
BOOL CImageWindow_Impl::FireEventKeyEvent(MSG *pMsg)
{
...
void CImageWindow_Impl::FireEventOnScroll()
{
	if(m_pEventHandler == NULL) return;

	m_pEventHandler->OnScroll();
}

void CImageWindow_Impl::FireEventOnZoom()
{
	if(m_pEventHandler == NULL) return;

	m_pEventHandler->OnZoom();
}

void CImageWindow_Impl::__ConvertViewToImagePos(long ptx, long pty, float &fX, float &fY)
{
	pty -= GetToolBarSizeY();
...
BOOL CImageWindow_Impl::__OnMouseWheel(UINT nFlags, short zDelta, CPoint point)
{
	if( m_szImage.cx <= 0 || m_szImage.cy <= 0 ) return FALSE;

	BOOL bRet = __Zoom(zDelta / WHEEL_DELTA, point);
...
BOOL CImageWindow_Impl::__OnMouseMove(UINT nFlags, CPoint point)
{
...
BOOL CImageWindow_Impl::__OnNavigate(UINT nFlags, CPoint point)
{
...
BOOL CImageWindow_Impl::__OnDisplayPosition(UINT nFlags, CPoint point)
{
...
BOOL CImageWindow_Impl::__OnDrag(UINT nFlags, CPoint point)
{
	BOOL bInvalidate = FALSE;
...
BOOL CImageWindow_Impl::__OnLButtonDrag(UINT nFlags, CPoint point)
{
...
BOOL CImageWindow_Impl::__OnRButtonDrag(UINT nFlags, CPoint point)
{
...
BOOL CImageWindow_Impl::__OnMinimapDrag(UINT nFlags, CPoint point)
{
...
/////////////////////////////////////////////////////////////////////////////
// CImageWindow_Impl::OnDraw - Drawing function
#define RECTWIDTH(x)	(x).Width()
#define RECTHEIGHT(x)	(x).Height()

void CImageWindow_Impl::OnPaint()
{
...
BOOL CImageWindow_Impl::PreCreateWindow(CREATESTRUCT &cs)
{
...
int CImageWindow_Impl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CImageWindow_Impl::GetScrollRange(int nBar, LPINT lpMinPos, LPINT lpMaxPos)
{
	switch(nBar)
	{
	case SB_VERT:
...
void CImageWindow_Impl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar)
{
...
void CImageWindow_Impl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar)
{
	int nNext, nMin, nMax;
...
BOOL CImageWindow_Impl::OnSetCursor(CWnd *pWnd, UINT nHitTest, UINT message)
{
...
void CImageWindow_Impl::OnLButtonDown(UINT nFlags, CPoint point)
{
	SetCapture();
...
void CImageWindow_Impl::OnLButtonUp(UINT nFlags, CPoint point)
{
	ReleaseCapture();
...
void CImageWindow_Impl::OnMouseMove(UINT nFlags, CPoint point)
{
	if(!m_bImageWindowEnable)
	{
...
BOOL CImageWindow_Impl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if(!m_bImageWindowEnable)
	{
...
void CImageWindow_Impl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
...
/////////////////////////////////////////////////////////////////////////////
// CImageWindow_Impl message handlers


//////////////////////////////////////////////////////////////////////////////
// Image Alloc , Set/Getimage and save/load
//////////////////////////////////////////////////////////////////////////////
BOOL CImageWindow_Impl::GetImage(const BYTE **ppbuf, long *width, long *height, long *depth, long *widthBytes)
{
...
void CImageWindow_Impl::OverlayShow(BOOL bShow)
{
...
void CImageWindow_Impl::OverlaySetLineWidth(short nLineWidth)
{
...
void CImageWindow_Impl::OverlayAddPoint(const CPoint *pPt, OLE_COLOR crColor)
{
	Overlay::Point sobj;
	sobj.fX = (float)pPt->x;
	sobj.fY = (float)pPt->y;
...
void CImageWindow_Impl::OverlayAddPoints(const CPoint *pPt, long nCount, OLE_COLOR crColor)
{
	Overlay::Point sobj;
	sobj.crColor = (COLORREF) crColor;
...
void CImageWindow_Impl::OverlayAddLine(const CPoint *pPt1, const CPoint *pPt2, OLE_COLOR crColor)
{
	Overlay::Line sobj;
...
void CImageWindow_Impl::OverlayAddRectangle(const CRect *pRect, OLE_COLOR crColor, float /*rot*/)
{
	if(pRect == NULL)
		return;
...
void CImageWindow_Impl::OverlayAddRectangle(const CPoint *pLT, const CPoint *pRT, const CPoint *pRB, const CPoint *pLB, OLE_COLOR crColor, float /*rot*/)
{
...
void CImageWindow_Impl::OverlayAddRectangles(const CRect *pRect, long nCount, OLE_COLOR crColor, float /*rot*/)
{
	if(pRect == NULL)
		return;
...
void CImageWindow_Impl::OverlayAddCircle(float fCenX, float fCenY, float fRadiusX, float fRadiusY, OLE_COLOR crColor, BOOL /*bScaleDepend*/)
{
...
void CImageWindow_Impl::OverlayAddFText(float fX, float fY, LPCTSTR str, OLE_COLOR crColor)
{
	Overlay::Text sobj;
...
void CImageWindow_Impl::OverlayAddText(const CPoint *pPt, LPCTSTR str, OLE_COLOR crColor)
{
...
void CImageWindow_Impl::OverlayAddTextEx(const CPoint *pPt, LPCTSTR str, LOGFONT *pFont, OLE_COLOR crColor)
{
	// 표시할 텍스트가 많아지면 GDI 리소스가 고갈되는 문제가 있음
	// 따라서, 폰트는 1 개로 지정한다.
	// 맨 처음 원소의 것을 사용.
...
void CImageWindow_Impl::OverlayRemoveLine()
{
	m_vecLine.clear();
...
void CImageWindow_Impl::OverlayRemoveRectangle()
{
	m_vecRect.clear();
...
void CImageWindow_Impl::OverlayRemovePoint()
{
	m_vecPoint.clear();
...
void CImageWindow_Impl::OverlayRemoveCircle()
{
	m_vecEllipse.clear();
...
void CImageWindow_Impl::OverlayRemoveText()
{
	m_vecText.clear();
...
void CImageWindow_Impl::CreateFont(int nTextFontHeight)
{
	TEXTMETRIC tm;
	CClientDC dc(this);
...
void CImageWindow_Impl::TransTracker2ImagePos(CRect rect, CRect *o_Rect)
{
...
void CImageWindow_Impl::TransImagePos2TrackerPos(float fLeft, float fTop, float fRight, float fBottom, CRect *o_Rect)
{
...
CPoint CImageWindow_Impl::TransBound2ImagePos(CPoint i_pt)
{
	i_pt.y -= GetToolBarSizeY();
	return ScreenToImage(i_pt);
}

CPoint CImageWindow_Impl::TransImagePos2Bound(float fX, float fY)
{
...
CSize CImageWindow_Impl::ImageToScreenDist(float fDist)
{
	return CSize(ImageToScreenCX(fDist), ImageToScreenCY(fDist));
}

CRect CImageWindow_Impl::ScreenToImageRect(const CRect &rt)
{
	return CRect(ScreenToImage(rt.TopLeft()), ScreenToImage(rt.BottomRight()));
}

CRect CImageWindow_Impl::ImageToScreenRect(const CRect &rt)
{
	return CRect(ImageToScreen(rt.TopLeft()), ImageToScreen(rt.BottomRight()));
}

CRect CImageWindow_Impl::ImageToScreenRect(float fLeft, float fTop, float fRight, float fBottom)
{
...
CRect CImageWindow_Impl::ClientToImageRect(const CRect &rt)
{
	return CRect(ClientToImage(rt.TopLeft()), ClientToImage(rt.BottomRight()));
}
CRect CImageWindow_Impl::ImageToClientRect(const CRect &rt)
{
...
void CImageWindow_Impl::DoDataExchange(CDataExchange *pDX)
{
	CDialog::DoDataExchange(pDX);
}

void CImageWindow_Impl::OnContextMenu(CWnd *pWnd, CPoint point)
{
...
void CImageWindow_Impl::CmdRouteMenu(CWnd *pWnd, CMenu *pPopupMenu)
{
	CCmdUI state;
...
void CImageWindow_Impl::OnZoomin()
{
...
void CImageWindow_Impl::OnZoomout()
{
	__Zoom(-1, CPoint(m_szScreen.cx / 2, m_szScreen.cy / 2));
...
void CImageWindow_Impl::OnZoom1x()
{
	//=================================
	// ZoomScale 변경
	//=================================
...
void CImageWindow_Impl::OnZoomwidthfit()
{
...
void CImageWindow_Impl::OnZoomimagefit()
{
	m_nDefaultZoomFitMode = ZOOMFIT_MODE_IMAGE;
...
void CImageWindow_Impl::OnZoomboundfit()
{
...
void CImageWindow_Impl::ExtZoomIn()
{
	OnZoomin();
}

void CImageWindow_Impl::ExtZoomOut()
{
	OnZoomout();
}

//---> 원섭 2005-07-04 [Overlay항목들을 모두 삭제한다.]
void CImageWindow_Impl::OverlayRemoveAll()
{
	m_vecPoint		.clear();
	m_vecLine		.clear();
	m_vecRect		.clear();
...
void CImageWindow_Impl::ExtMove(long nImageCenterX, long nImageCenterY)
{
	ExtMove((float)nImageCenterX, (float)nImageCenterY);
}

void CImageWindow_Impl::ExtMove(float fImageCenterX, float fImageCenterY)
{
	__Scroll(	-__GetScreenOffsetX(m_szScreen.cx / 2.f, fImageCenterX),
	            -__GetScreenOffsetY(m_szScreen.cy / 2.f, fImageCenterY));
}

void CImageWindow_Impl::ExtPosSet(long nScreenOffsetX, long nScreenOffsetY)
{
	m_ptScreenOffset.x = nScreenOffsetX;
	m_ptScreenOffset.y = nScreenOffsetY;

	__UpdateMinimapPostiion();
	__UpdateDisplayInformation();
}

void CImageWindow_Impl::ExtPosGet(long& nScreenOffsetX, long& nScreenOffsetY)
{
	nScreenOffsetX = m_ptScreenOffset.x;
	nScreenOffsetY = m_ptScreenOffset.y;
}

void CImageWindow_Impl::SetDisplayImage(const BYTE *buf, long nWidth, long nHeight, long nWidthBytes, long nDepth)
{
	...
	{
		__UpdateScale();
		__csDrawImage.Unlock();
		return;
	}

	CDC* pDC = GetDC();
	m_hDIB = DIBAlloc(pDC, m_imgOriginalImage.nWidth, m_imgOriginalImage.nHeight, m_imgOriginalImage.nBpp, TRUE);
	memcpy((BYTE*)m_pPixelRef, m_imgOriginalImage.pbyImage, m_imgOriginalImage.nWidthBytes * m_imgOriginalImage.nHeight);
	ReleaseDC(pDC);

	//CDC* pDC = GetDC();

	//m_hBitmapDC	= __MakeDDB(pDC,
...
void CImageWindow_Impl::UpdateScrollRange()
{
...
void CImageWindow_Impl::UpdateScrollShow()
{
...
void CImageWindow_Impl::__UpdateScrollInfoX()
{
...
void CImageWindow_Impl::__UpdateScrollInfoY()
{
...
BOOL CImageWindow_Impl::__IsHScrollAvailable()
{
...
BOOL CImageWindow_Impl::__IsVScrollAvailable()
{
	if(!m_imgShowSelImage.pbyImage) return FALSE;
...
void CImageWindow_Impl::ROIShow()
{
	m_isROIDraw = TRUE;
	Invalidate(FALSE);
}

void CImageWindow_Impl::ROIHide()
{
	m_isROIDraw = FALSE;
	Invalidate(FALSE);
}

long CImageWindow_Impl::ROIAdd(const CRect& rtROI, OLE_COLOR crColor, LPCTSTR pstrName, short nLineWidth, BOOL isMovalble, BOOL isResizable)
{
	ROIDrawPara para;
...
long CImageWindow_Impl::ROIAdd(const ROIData& Data, OLE_COLOR crColor, LPCTSTR pstrName, short nLineWidth, BOOL isMovalble, BOOL isResizable)
{
	ROIDrawPara para;
...
void CImageWindow_Impl::ROISetMinSize(long nMinSizeX, long nMinSizeY)
{
...
void CImageWindow_Impl::ROIRemoveAll()
{
	m_nSelROI = -1;
	m_nSelKnob = -1;
...
BOOL CImageWindow_Impl::ROIGet(long nID, CRect *prtROI)
{
...
ROIData CImageWindow_Impl::ROIGet(long nID) const
{
...
BOOL CImageWindow_Impl::ROISet(long nID, const ROIData& Data, OLE_COLOR crColor, LPCTSTR pstrName, short nLineWidth, BOOL isMovalble, BOOL isResizable)
{
	if( OutOfRange(m_vecROIGeo, nID) ) return FALSE;
	...
	if( pROIGeo && pROIDraw )
	{
		safeRelease(m_vecROIDraw[nID]);
		safeRelease(m_vecROIGeo[nID]);

		m_vecROIGeo[nID] = pROIGeo;
		m_vecROIDraw[nID] = pROIDraw;
...
BOOL CImageWindow_Impl::ROIDel(long nID)
{
	if( OutOfRange(m_vecROIGeo, nID) ) return FALSE;
	...
	m_vecROIDraw.erase(m_vecROIDraw.begin() + nID);
	m_vecROIGeo.erase(m_vecROIGeo.begin() + nID);

	return TRUE;
}

BOOL CImageWindow_Impl::ROISel(long nID)
{
...
long CImageWindow_Impl::ROICount() const
{
	return (long)m_vecROIGeo.size();
}

long CImageWindow_Impl::ROIGetCurSel() const
{
	return m_nSelROI;
}

void CImageWindow_Impl::OverlayAddLineEx(const CPoint *pPt1, const CPoint *pPt2, OLE_COLOR crColor, long nPenStyle)
{
...
void CImageWindow_Impl::OverlayAddRectangleEx(const CRect *pRect, OLE_COLOR crColor, long nPenStyle)
{
	Overlay::Rect sobj;
...
void CImageWindow_Impl::ExtZoomScaleGet(double *pScaleH, double *pScaleV)
{
...
// =========================================================
/** @fn		void CImageWindow_Impl::ExtZoomScale(double fScaleH, double fScaleV)
@brief		임의의 스케일로 변환한다.
...
*/ // ======================================================
void CImageWindow_Impl::ExtZoomScale(double fScaleH, double fScaleV)
{
...
void CImageWindow_Impl::ExtZoomROIGet(float *pLeft, float *pTop, float *pRight, float *pBottom)
{
...
void CImageWindow_Impl::ExtZoomROISet(float fLeft, float fTop, float fRight, float fBottom)
{
...
void CImageWindow_Impl::OnSize(UINT nType, int cx, int cy)
{
...
void CImageWindow_Impl::ExtZoomFit(short nZoomFitMode)
{
	switch(nZoomFitMode)
	{
...
void CImageWindow_Impl::OverlayAddFPoint(float fX, float fY, OLE_COLOR crColor)
{
	// TODO: Add your dispatch handler code here
	Overlay::Point sobj;
	sobj.fX = fX;
...
void CImageWindow_Impl::OverlayAddFLine(float fX1, float fY1, float fX2, float fY2, OLE_COLOR crColor)
{
	// TODO: Add your dispatch handler code here
	Overlay::Line sobj;
	sobj.fX1 = fX1;
...
void CImageWindow_Impl::OverlayAddFRectangle(float fLeft, float fTop, float fRight, float fBottom, OLE_COLOR crColor)
{
	// TODO: Add your dispatch handler code here
	Overlay::Rect sobj;
...
void CImageWindow_Impl::OverlayAddArcDoughnut(float fCenX, float fCenY, float fStartAngle, float fEndAngle, float fRadiusX, float fRadiusY, float fWidth, OLE_COLOR crColor)
{
...
void CImageWindow_Impl::OverlayAddArcRing(float fCenX, float fCenY, float fStartAngle, float fEndAngle, float fInnerLength, float fOuterLength, OLE_COLOR crColor)
{
...
void CImageWindow_Impl::SetDisplayWindow(CWnd *pDialog)
{
...
void CImageWindow_Impl::SetDisplayImage_Palette(const BYTE *buf, long nWidth, long nHeight, long nWidthBytes, long nDepth, OLE_COLOR *pPaletteColor)
{
	SWindow_Image previmg = m_imgOriginalImage;
...
void CImageWindow_Impl::EnableDisplayHighQuality(BOOL bEnable)
{
	m_iStretchMode = bEnable ? HALFTONE : COLORONCOLOR;
}

void CImageWindow_Impl::OnDisplaycurrentpos()
{
...
void CImageWindow_Impl::uDrawCurrentPosition( CDC *pdc )
{
...
void CImageWindow_Impl::uDrawPixelMeter( CDC *pdc )
{
...
void CImageWindow_Impl::uDrawMinimap(CDC *pdc, const CRect &rt)
{
	if( !pdc ) return;
...
void CImageWindow_Impl::OnCrossmark()
{
	//static BYTE ppp[] = {
...
void CImageWindow_Impl::OnUpdateCrossmark(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_bDisplayCrossMark);
}

void CImageWindow_Impl::OnRemovealloverlay()
{
	OverlayRemoveAll();
}

void CImageWindow_Impl::OnUpdateRemovealloverlay(CCmdUI *pCmdUI)
{
...
void CImageWindow_Impl::OnZoom2x()
{
	m_fScaleX = 2.f;
	m_fScaleY = 2.f;
...
void CImageWindow_Impl::OnZoom5x()
{
	m_fScaleX = 5.f;
	m_fScaleY = 5.f;
...
void CImageWindow_Impl::OnZoom10x()
{
	m_fScaleX = 10.f;
	m_fScaleY = 10.f;
...
void CImageWindow_Impl::OnRButtonDown(UINT nFlags, CPoint point)
{
	SetCapture();
...
void CImageWindow_Impl::OnRButtonUp(UINT nFlags, CPoint point)
{
	ReleaseCapture();
...
void CImageWindow_Impl::EnableMinimap(BOOL bEnable, CSize size)
{
...
void CImageWindow_Impl::SetDefaultZoomFitMode(long nZoomFitMode)
{
	switch(nZoomFitMode)
	{
	case 1:
...
void CImageWindow_Impl::AddPopupMenuExternal(long nID, LPCTSTR pstrItem)
{
...
void CImageWindow_Impl::RemovePopupMenuExternal(long nID)
{
	if( __mapExpandMenu.find(nID) == __mapExpandMenu.end() )
	{
		ASSERT(0);
...
void CImageWindow_Impl::ModifyPopupMenuExternal(long nID, short bEnable, short bChecked)
{
	if( __mapExpandMenu.find(nID) == __mapExpandMenu.end() )
	{
		ASSERT(0);
...
void CImageWindow_Impl::__AppendMenuExternal(CMenu *pMenu)
{
...
void CImageWindow_Impl::__DeleteMenuExternal(CMenu *pMenu)
{
...
void CImageWindow_Impl::__SetCheckPopupMenu(CMenu *pMenu)
{
...
void CImageWindow_Impl::__SetCheckShowToolBarPopupMenu(CMenu *pMenu)
{
...
void CImageWindow_Impl::__SetCheckShowMiniMapPopupMenu(CMenu *pMenu)
{
...
void CImageWindow_Impl::__SetCheckZoomFitModePopupMenu(CMenu *pMenu)
{
	BOOL bCheck = TRUE;
...
void CImageWindow_Impl::__SetCheckShowPixelMeterPopupMenu(CMenu *pMenu)
{
...
UINT CImageWindow_Impl::__GetZoomFitModeID(enumZoomFitMode nZoomFitMode)
{
	switch(nZoomFitMode)
	{
	case ZOOMFIT_MODE_WIDTH:
...
BOOL CImageWindow_Impl::OnCommand(WPARAM wParam, LPARAM lParam)
{
...
void CImageWindow_Impl::__UpdateScale()
{
...
void CImageWindow_Impl::__UpdateDisplayInformation()
{
...
CPoint CImageWindow_Impl::ScreenToImage(CPoint point)
{
...
CPoint CImageWindow_Impl::ImageToScreen(CPoint point)
{
...
CPoint CImageWindow_Impl::ClientToImage(CPoint point)
{
...
CPoint CImageWindow_Impl::ImageToClient(CPoint point)
{
...
CPoint CImageWindow_Impl::ImageToScreen(float x, float y)
{	
	CPoint pt;
	if(m_bUseRangeImage)
	{
...
CPoint CImageWindow_Impl::ImageToClient(float x, float y)
{
...
HBITMAP CImageWindow_Impl::DIBAlloc(CDC* pDC, long width, long height, long bpp, BOOL bTopToBottom)
{
...
void CImageWindow_Impl::DIBRelease()
{
	DeleteObject(m_hDIB);
...
void CImageWindow_Impl::__DrawImage(CDC *pDC)
{
	if( !pDC || !m_hDIB) return;

	CSingleLock lock(&__csDrawImage, TRUE);
	...
	HBITMAP hOldBitmap = (HBITMAP)MemDC.SelectObject(m_hDIB);
...
HBITMAP CImageWindow_Impl::__MakeDDB(CDC *pDC, const unsigned char *imgdata, long width, long height, long widthBytes, int bpp, BOOL bTopToBottom, OLE_COLOR *pPaletteColor)
{
...
void CImageWindow_Impl::__DrawTrace(CDC *pMemDC, CMouseTrace *pMouseTrace)
{
...
void CImageWindow_Impl::__TraceBegin(const CPoint &ptImage)
{
...
CRect CImageWindow_Impl::__Trace(const CPoint &ptImage)
{
...
void CImageWindow_Impl::__TraceEnd()
{
	if( !__IsTraceEnable() || !m_pMouseTrace ) return;
...
void CImageWindow_Impl::__TraceExport()
{
...
void CImageWindow_Impl::SetTraceColor(BYTE a, BYTE r, BYTE g, BYTE b)
{
	m_argbTrace = Color(a, r, g, b);
}

void CImageWindow_Impl::SetTraceWidth(int nTraceWidth)
{
	m_nTraceWidth = nTraceWidth;
}

void CImageWindow_Impl::GetMouseTrace(CPoint **ppVertice, long *pnVertice)
{
...
float	CImageWindow_Impl::__GetScaleWidthFit()
{
...
float	CImageWindow_Impl::__GetScaleHeightFit()
{
...
float	CImageWindow_Impl::__GetScaleImageFit()
{
	int cxImage = m_szImage.cx;
	int cyImage = m_szImage.cy;
...
CSize CImageWindow_Impl::__GetScreenSize()
{
	CRect rtClient;
	GetClientRect(&rtClient);
...
CSize CImageWindow_Impl::__GetImageSize()
{
	return CSize(m_imgShowSelImage.nWidth, m_imgShowSelImage.nHeight);
}

BOOL CImageWindow_Impl::__IsImageSizeUnderScreen()
{
	return m_szScreen.cx > m_szImage.cx && m_szScreen.cy > m_szImage.cy;
}

long CImageWindow_Impl::__FindZoomScaleIndex(float fScale)
{
...
void CImageWindow_Impl::__Scroll(long nScrollPosX, long nScrollPosY)
{
...
void CImageWindow_Impl::__ScrollLimit(long &nScrollPosX, long &nScrollPosY)
{
...
void CImageWindow_Impl::__ZoomFitWidth()
{
	CRect rtWindow;
...
void CImageWindow_Impl::__ZoomFitImage()
{
	CRect rtWindow;
	GetWindowRect(&rtWindow);
...
void CImageWindow_Impl::__ZoomFitBound()
{
	CRect rtWindow;
	GetWindowRect(&rtWindow);
...
BOOL CImageWindow_Impl::SetDisplayRange(BOOL bUse, long nLeft, long nTop, long nRight, long nBottom)
{
...
void CImageWindow_Impl::EnablePixelMeter(BOOL bEnable)
{
...
void CImageWindow_Impl::SetPixelMeterUnit(float fOnePixelUnit, LPCTSTR lpszUnitName)
{
...
void CImageWindow_Impl::SetPixelMeterBegin(const CPoint &ptBegin)
{
...
void CImageWindow_Impl::SetPixelMeterEnd(const CPoint &ptEnd)
{
...
void CImageWindow_Impl::Screenshot(BOOL bShotImage, BOOL bShotROI, CBitmap& o_ddb)
{
	CRect rcBounds(0, 0, 0, 0);
	this->GetClientRect(&rcBounds);
...
void CImageWindow_Impl::OnShowtoolbar()
{
...
BOOL CImageWindow_Impl::OnToolTipText(UINT, NMHDR *pNMHDR, LRESULT *pResult)
{
...
BOOL CImageWindow_Impl::__CreateToolBar()
{
...
BOOL CImageWindow_Impl::__IsScrollRangeUpdating()
{
	return m_bScrollRangeUpdating;
}

void CImageWindow_Impl::__SetScrollRangeUpdating(BOOL bUpdating)
{
	m_bScrollRangeUpdating = bUpdating;
}

void CImageWindow_Impl::__MoveView(int x, int y, int cx, int cy)
{
	UpdateScrollRange();
	__UpdateDisplayInformation();
	__UpdateMinimapPostiion();
...
BOOL CImageWindow_Impl::OnInitDialog()
{
...
void CImageWindow_Impl::ShowToolBar(BOOL bShow)
{
	m_wndToolBar.ShowWindow(bShow ? SW_SHOW : SW_HIDE);
}

int CImageWindow_Impl::GetToolBarSizeY()
{
...
BOOL CImageWindow_Impl::__IsMinimapShowing()
{
	return __IsHScrollAvailable() || __IsVScrollAvailable();
}

BOOL CImageWindow_Impl::__UpdateMinimapPostiion()
{
...
BOOL CImageWindow_Impl::__IsMinimapEnabled()
{
	return __bMinimapEnabled;
}

CString CImageWindow_Impl::__MakeImagePositionString(CPoint point)
{
...
BOOL CImageWindow_Impl::__Zoom(long nDiffIndex, CPoint point)
{
...
BOOL CImageWindow_Impl::__Zoom(float fScaleX, float fScaleY, CPoint point)
{
...
BOOL CImageWindow_Impl::PreTranslateMessage(MSG *pMsg)
{
...
void CImageWindow_Impl::OnPopupShowminimap()
{
	EnableMinimap(!__IsMinimapEnabled(), CSize(100, 100));
}

void CImageWindow_Impl::OnPopupShowpixelmeter()
{
	EnablePixelMeter(!m_PixelMeter.IsEnable());
}


void CImageWindow_Impl::OnExportFile()
{
	...
	if(nImageSize > 0)
	{
		...
		//IV_dataImage* pImage = IV_DATAIMAGE_FACTORY::Create(IV_DATAIMAGE_FACTORY::BPPToIITYPE(m_imgShowSelImage.nBpp));

		//pImage->Alias(m_imgShowSelImage.nWidthBytes, m_imgShowSelImage.nWidth, m_imgShowSelImage.nHeight, pSaveData);

		//if(strExt == _T("BMP"))
		//{
		//	pImage->SaveToBMP(dlg.GetPathName());
		//}
		//else if(strExt == _T("PNG"))
		//{
		//	pImage->SaveToPNG(dlg.GetPathName());
		//}
		//else if(strExt == _T("JPG") || strExt == _T("JPEG"))
		//{
		//	pImage->SaveToJPEG(dlg.GetPathName());
		//}
...
void CImageWindow_Impl::OnExportClipboard()
{
	CDC*		pDC		= GetDC();
...
void CImageWindow_Impl::OnPopupScreenshot()
{
	CBitmap Bitmap;
...
BOOL CImageWindow_Impl::IsImageValid() const
{
	return (m_imgShowSelImage.pbyImage && m_imgShowSelImage.nWidth > 0 && m_imgShowSelImage.nHeight > 0);
}

void CImageWindow_Impl::OnUpdatePopupScreenshot(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(IsImageValid());
}

void CImageWindow_Impl::OnUpdateExportFile(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(IsImageValid());
}

void CImageWindow_Impl::OnUpdateExportClipboard(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(IsImageValid());
}
