...
{
	//  clean up the memory.
	if (m_pptData != nullptr)
	{
		delete m_pptData;
		m_pptData = nullptr;
	}
	if (m_pptScreen != nullptr)
	{
		delete m_pptScreen;
		m_pptScreen = nullptr;
	}
...
CRect CLinePlot::DrawFramework(CDC *pDC, CRect rcBounds, int nFontHeight, bool oIsPrinting)
{
	...
	if (m_lstPlotData.size() > 1)
	{
		...
		m_rcKeyArea.top = nYCenter - ((siScale.cy + 4) * (long)m_lstPlotData.size()) / 2;
		m_rcKeyArea.bottom = nYCenter + ((siScale.cy + 4) * (long)m_lstPlotData.size()) / 2;
	...
	CString szXFormat;
	szXFormat.Format(_T("%%0.%uf"), m_uiXPrecision);
	CString szYFormat;
	...
	*/
	...
	if (rcPlotArea.left < rcPlotArea.right)
	{
		...
		POINT ptXScale; ptXScale.x = 0; ptXScale.y = 0;
		POINT ptYScale; ptYScale.x = 0; ptYScale.y = 0;
		CString szScale;
		pfonOld = pDC->SelectObject(&fonScale);
...
int CLinePlot::Add(CString szName, COLORREF crColor, enumPlotStyle nStyle, FLOATPOINT *pptData, UINT uiPointCount)
{
	...
	m_nSelected = (int)m_lstPlotData.size() - 1;
...
void CLinePlot::HideEditWindow()
{
	...
	if (m_edtDataEntry.IsWindowVisible() == TRUE)
	{
		...
		float eValue  = (float)(_ttof(szText.GetBuffer(1)));
