...
CSliderCtrlEx::CSliderCtrlEx()
{
	...
	m_Callback = nullptr;	// pointer to function to call upon slider update
...
{
	int loopMax = (int)colorList.GetSize();	// number of color ranges to process
...
BOOL CSliderCtrlEx::AddColor(int nLow, int nHigh, COLORREF strColor, COLORREF endColor)
{
	...
	if((nLow < l) || (nHigh > h))	// sanity check
	{
		...
		{
			CString msg;
			msg.Format(_T("Bad range (%d,%d) given, must be within (%d,%d)"), nLow,nHigh,l,h);
